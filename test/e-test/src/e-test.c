...
int main(int argc, char *argv[]){
  ...
  if (argc < 7){
	...
	for (j=col0; j<(col0+cols); j++) {   
	  e_load_group.srecFile, &dev, i, j, 1, 1, E_TRUE);
	}
      }    
    }  
    else{
      e_load_group.srecFile, &dev, row0, col0, rows, cols, E_TRUE);
    }
    //Checking the test
    for (i=row0; i<(row0+rows); i++) {
      for (j=col0; j<(col0+cols); j++) {   
...
void usage(){
 printf("-----------------------------------------------\n");
  ...
  printf("Usage:    e-test <row> <col> <rows> <cols> .srec>\n");
  printf("Example:  e-test 0 0 4 4 1 my.srec\n");
  printf("\n");
  printf("Options:\n");
